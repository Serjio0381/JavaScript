...
// }                         
 ...
 // нверное эту задачу попробую решить с помощью тернарного оператора 

// let num1 = Number(prompt(`Введите первое число `));
// (num1 <= 1) ? alert(`число соответствует`) : alert(`число больше 1`);
// let num2 = Number(prompt(`Введите второе число `));
// (num2 >= 3) ? alert(`число соответствует`) :  alert(`число меньше 3`); 
...
// let day = Number(prompt(`Введите любое число месяца`));
// if (day <= 10 & day > 0) {
//     alert(`первая декада`);
// } 
...
// let sot = Math.floor(n / 100);
// let des = Math.floor(n / 10 % 10);
// let ed = Math.floor(n % 10);
// alert(`сотен ${sot}, десятков ${des}, единиц ${ed} `); 
